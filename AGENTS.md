...
4. Dispatch to the selected statistical test (each takes `X_group, X_rest` → `scores, pvals`).
   `deseq2` is the exception: it receives the whole `adata` plus `replicate_col`/`covariates` and
   aggregates cells into pseudo-bulk samples itself, so it also skips the `find_all_markers()`
   precomputation step.
